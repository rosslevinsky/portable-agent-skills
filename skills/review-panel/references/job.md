...
2. **Root and exclusions.** The directory the sweep reads — written into the job as an
   **absolute** path — and any paths under it to leave out. An excluded path is still
   classified: the report lists it under *not read*. Where the owner wants a **subset** of a
...
4. **The lenses.** How each reader reads the same files. The default is two: bottom-up from
   ...
   area is read by both model slots; an area may carry its own list.
...
confirm. A job whose answer is not a finding (a question, a design opinion, "is this
approach right") stops here with that reason, before anything is dispatched: v1's unit of
output is a finding, and dispatching such a job produces findings about the wrong thing.
...
but the defect-audit test still applies — text that asks a question rather than naming what
to find is refused with that reason.
...
in the run directory rather than something a reader has to work out. Exclusions still apply
to the listed set and still have to match something, and `.git` is not listable — the

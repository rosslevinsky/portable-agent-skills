...
name: plan-init-v1
description: >
  Create a plan document using the superseded v1 workflow — it writes no `Format: v2`
  marker and no execution.md tracker. Use when the user invokes /plan-init-v1, or is
  continuing an existing v1 plan; new work belongs to /plan-init. Interviews the user
  to fill gaps, explores the codebase for context, then produces a master plan with
...
The user's argument (everything after `/plan-init-v1`) is the task description. If no
...
based on the task description and record each assumption explicitly in the plan's
**Assumptions** section (Step 6).

---

## Step 3 — Explore the codebase

First work through the framework/library standards and quality-gate checks in the
subsections below — they take precedence over any patterns you'd infer from code, so
they must precede exploration. Then explore the parts of the codebase relevant to the
task. Identify:
...
**not** overwrite it — a plan there is prior work. Either choose a more specific slug
(and note the change) or, if it looks like the same task, stop and ask the user
whether to resume or replace the existing plan.
...
_(Autonomous mode only — record each assumption made in place of a user answer;
omit this section entirely when the user answered the interview.)_

- <assumption made in place of a user answer>
- <...>
...
_Phases: not yet broken down — run /plan-phase-v1 to generate phase documents._
```
